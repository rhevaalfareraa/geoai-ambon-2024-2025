async function loadGeoJSON(path) {
  ...
  if (!d || d.type !== "FeatureCollection" || !Array.isArray(d.features))
    throw new Error(`GeoJSON tidak valid: ${path}`);
  return d;
}
...
function loaderFor(path) {
  if (path.endsWith(".geojson")) return loadGeoJSON(path);
  if (path.endsWith(".json")) return loadJSON(path);
  return loadCSV(path);
}
async function loadDataKey(key) {
  ...
  const path = APP_CONFIG.paths[key];
  if (!path) return null;
  const job = loaderFor(path)
    .then((v) => {
      APP_STATE.data[key] = v;
      return v;
    })
    .catch((e) => {
      console.error(key, e);
      APP_STATE.data[key] = null;
      return null;
    })
    .finally(() => APP_STATE.loadingKeys.delete(key));
  APP_STATE.loadingKeys.set(key, job);
  return job;
}
async function loadKeys(keys) {
  await Promise.allSettled(keys.map(loadDataKey));
