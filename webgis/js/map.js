...
function initializeMap() {
  ...
  webgisMap.on("zoomend moveend resize", () => {
    if (swipeActive)
      requestAnimationFrame(() => updateNativeSwipe(swipePercent));
  });
...
function addBasemapSwitcher() {
  const Control = L.Control.extend({
    ...
    onAdd() {
      ...
      wrap.innerHTML =
        '<button type="button" data-base="map" class="active">Map</button><button type="button" data-base="osm">OSM</button><button type="button" data-base="satellite">Satellite</button>';
...
async function loadChangeRaster() {
  ...
  const response = await fetch(
    "./data/raster/RF_Change_Map_Ambon_2024_2025.tif",
    { cache: "no-store" },
  );
  if (!response.ok)
    throw new Error(`GeoTIFF Change Map gagal dimuat: HTTP ${response.status}`);
...
function districtNameForFeature(feature) {
  try {
    ...
    const district = APP_STATE.data.districts.features.find((item) => {
      try {
        return turf.booleanPointInPolygon(point, item);
      } catch (_) {
        return false;
      }
    });
...
function setSwipeLabels(show) {
  document
    .getElementById("swipe-label-left")
    ?.classList.toggle("hidden", !show);
  document
    .getElementById("swipe-label-right")
    ?.classList.toggle("hidden", !show);
  document
    .getElementById("native-swipe-control")
    ?.classList.toggle("hidden", !show);
}
...
function makeSwipePolygonLayer(key, data, paneName, styleOverride = {}) {
  ...
  return L.geoJSON(data, {
    ...
    onEachFeature: (feature, layer) => {
      ...
      layer.bindTooltip(
        `${title} • ${safeNumber(feature.properties?.area_ha)} ha`,
        { sticky: true },
      );
    },
  });
...
function updateNativeSwipe(value) {
  ...
  if (compareText)
    compareText.textContent = `2024 ${swipePercent}% • 2025 ${100 - swipePercent}%`;
}
...
function initializeNativeSwipeInput() {
  ...
  range.addEventListener("input", (event) =>
    updateNativeSwipe(event.target.value),
  );
  range.addEventListener("pointerdown", (event) => event.stopPropagation());
  range.addEventListener("touchstart", (event) => event.stopPropagation(), {
    passive: true,
  });
}
...
async function enableSwipeComparison() {
  ...
  try {
    ...
    const leftLoss = APP_STATE.data.loss
      ? makeSwipePolygonLayer("loss", APP_STATE.data.loss, "swipe-left-pane", {
          fillColor: "#e74c3c",
          color: "#a93226",
          fillOpacity: 0.96,
          weight: 1.1,
    ...
    const rightGain = APP_STATE.data.gain
      ? makeSwipePolygonLayer("gain", APP_STATE.data.gain, "swipe-right-pane", {
          fillColor: "#f1c40f",
          color: "#b7950b",
          fillOpacity: 0.98,
          weight: 1.1,
    ...
    swipeLeftLayer = L.featureGroup(
      [leftVegetation, leftLoss].filter(Boolean),
    ).addTo(webgisMap);
    swipeRightLayer = L.featureGroup(
      [rightVegetation, rightGain].filter(Boolean),
    ).addTo(webgisMap);
    ...
    if (leftLabel)
      leftLabel.innerHTML =
        "<b>Vegetasi 2024</b><small>Loss ditandai merah</small>";
    if (rightLabel)
      rightLabel.innerHTML =
        "<b>Vegetasi 2025</b><small>Gain ditandai kuning</small>";
    ...
    if (APP_STATE.layers.boundary?.getBounds) {
      webgisMap.fitBounds(APP_STATE.layers.boundary.getBounds(), {
        padding: [10, 10],
      });
    }
...
function disableSwipeComparison(restoreChange = true) {
  [swipeLeftLayer, swipeRightLayer].forEach((layer) => {
...
function polygonPopup(feature, key) {
  ...
  if (key === "boundary")
    return `<table class="popup-table"><tr><td>Wilayah</td><td><b>${safeValue(p.NAME_2, "Kota Ambon")}</b></td></tr><tr><td>Provinsi</td><td>${safeValue(p.NAME_1, "Maluku")}</td></tr><tr><td>Tipe</td><td>${safeValue(p.TYPE_2, "Kota")}</td></tr><tr><td>Geometry</td><td>${feature.geometry?.type || "-"}</td></tr></table>`;
  if (key === "districts")
    return `<table class="popup-table"><tr><td>Kecamatan</td><td><b>${safeValue(p.NAME_3)}</b></td></tr><tr><td>Kota</td><td>${safeValue(p.NAME_2, "Ambon")}</td></tr><tr><td>Provinsi</td><td>${safeValue(p.NAME_1, "Maluku")}</td></tr><tr><td>Geometry</td><td>${feature.geometry?.type || "-"}</td></tr></table>`;
  const area = num(p.area_ha),
    coords = featureCenter(feature);
  const cityArea = num(
    getSummary()?.luas_kota_ha,
    APP_CONFIG.fallback.cityArea,
  );
...
function featureCenter(feature) {
  try {
    return turflessCenter(feature.geometry);
  } catch {
    return null;
  }
}
function turflessCenter(g) {
  let pts = [];
  const walk = (a) => {
...
function layerOpacity(key) {
  if (key === "vegetation2024")
    return num(document.getElementById("compare-slider")?.value, 50) / 100;
  if (key === "vegetation2025")
    return 1 - num(document.getElementById("compare-slider")?.value, 50) / 100;
  if (key === "loss") return 0.84;
  if (key === "gain") return 0.82;
  return 0.45;
}

function buildPriorityZones() {
  const features = [...(APP_STATE.data.loss?.features || [])].sort(
    (a, b) => num(b.properties?.area_ha) - num(a.properties?.area_ha),
  );
  ...
  return {
    ...
    features: features.map((f, i) => {
      const area = num(f.properties?.area_ha),
        ratio = max ? area / max : 0;
      const level =
        ratio >= 0.5 ? "Tinggi" : ratio >= 0.2 ? "Sedang" : "Rendah";
      return {
        ...f,
        properties: {
          ...(f.properties || {}),
          ...
          priority_rank: i + 1,
          priority_basis: "Ukuran patch loss relatif",
        },
      };
    }),
  };
...
function makeLayer(key, data) {
  if (key === "priorityZones") {
    ...
    return L.geoJSON(data, {
      style: (f) => ({
        color: colorByLevel[f.properties?.priority_level] || "#c0392b",
        weight: 1.5,
        fillColor: colorByLevel[f.properties?.priority_level] || "#c0392b",
        fillOpacity: 0.55,
        dashArray: f.properties?.priority_level === "Rendah" ? "4 3" : null,
      }),
      onEachFeature: (f, l) => {
        l.bindPopup(priorityPopup(f));
        l.bindTooltip(
          `Prioritas ${safeValue(f.properties?.priority_level)} • ${safeNumber(f.properties?.area_ha)} ha`,
          { sticky: true },
        );
      },
    });
  ...
  if (
    ...
    return L.geoJSON(data, {
      pointToLayer: (f, ll) => {
        ...
        let color;
        if (key.startsWith("groundTruth"))
          color =
            Number(p.class) === 1
              ? APP_CONFIG.colors.groundTruthVeg
              : APP_CONFIG.colors.groundTruthNon;
        else
          color =
            key === "featureSamples2024"
              ? APP_CONFIG.colors.feature2024
              : APP_CONFIG.colors.feature2025;
        return L.circleMarker(ll, {
          radius: key.startsWith("groundTruth") ? 5 : 4,
          color: "#fff",
          weight: 1,
          fillColor: color,
          fillOpacity: 0.88,
      ...
      onEachFeature: (f, l) => {
        l.bindPopup(pointPopup(f, key));
        l.bindTooltip(
          `${safeValue(f.properties?.sample_id)} • ${safeValue(f.properties?.label)}`,
          { sticky: true },
        );
      },
    });
  }
  const c = APP_CONFIG.colors[key];
  const lineOnly = ["boundary", "districts"].includes(key);
  return L.geoJSON(data, {
    style: () => ({
      color:
        key === "boundary" ? "#102c3a" : key === "districts" ? "#8bc34a" : c,
      weight: key === "boundary" ? 2.4 : key === "districts" ? 1.1 : 1,
      opacity: 1,
      fillColor: c,
      fillOpacity: lineOnly ? 0 : layerOpacity(key),
      dashArray: null,
    }),
    onEachFeature: (f, l) => {
      l.bindPopup(polygonPopup(f, key));
      if (key === "districts")
        l.bindTooltip(safeValue(f.properties?.NAME_3), { sticky: true });
      else if (!lineOnly)
        l.bindTooltip(
          `${safeValue(f.properties?.category)} • ${safeNumber(f.properties?.area_ha)} ha`,
          { sticky: true },
        );
      if (!lineOnly)
        l.on({
          mouseover: (e) =>
            e.target.setStyle({
              ...
              fillOpacity: Math.min(
                0.95,
                (e.target.options.fillOpacity || 0.4) + 0.12,
              ),
            }),
          mouseout: (e) => APP_STATE.layers[key].resetStyle(e.target),
        });
    },
  });
}
...
function findDistrict(name) {
  ...
  return (
    features.find(
      (feature) => getDistrictName(feature).toLowerCase() === normalized,
    ) || null
  );
}
...
function removeSelectedDistrictBoundary() {
  if (
    selectedDistrictBoundaryLayer &&
    webgisMap.hasLayer(selectedDistrictBoundaryLayer)
  ) {
...
function showOnlySelectedDistrictBoundary(districtFeature) {
  ...
  if (selectedDistrictBoundaryLayer.getBounds().isValid()) {
    webgisMap.fitBounds(selectedDistrictBoundaryLayer.getBounds(), {
      padding: [24, 24],
      maxZoom: 13,
    });
  }
...
async function renderMapLayers() {
  ["boundary", "districts"].forEach((k) => {
    if (APP_STATE.data[k])
      APP_STATE.layers[k] = makeLayer(k, APP_STATE.data[k]);
  });
  renderLayerToggles();
  await toggleLayer("boundary", true);
  await toggleLayer("districts", true);
  ...
  if (b?.getBounds?.().isValid()) {
    webgisMap.invalidateSize(false);
    webgisMap.fitBounds(b.getBounds(), {
...
async function ensureLayer(key) {
  ...
  if (key === "changeMap") {
    ...
    const layer = L.geoJSON(data, {
      style: (feature) => {
        const p = feature.properties || {};
        const code = Number(
          p.change_code ?? p.class_code ?? p.class ?? p.raster_value ?? p.label,
        );
  ...
  if (supportedKeys.includes(key)) {
    ...
    if (!data || data.type !== "FeatureCollection") {
      throw new Error(
        `GeoJSON ${LAYER_META[key]?.[0] || key} tidak valid atau tidak ditemukan.`,
      );
    }
...
async function toggleLayer(key, on) {
  ...
  try {
    ...
    if (on && key === "changeMap") {
      ...
      if (!layer)
        throw new Error("GeoJSON Change Map tidak menghasilkan layer");
...
function updateLegend() {
  ...
  const active = [...APP_STATE.activeOverlays].filter(
    (key) => !["boundary", "districts"].includes(key),
  );
...
function setMapStatus(message, show) {
  const el = document.getElementById("map-status");
  if (!el) return;
  el.style.display = show ? "flex" : "none";
  if (show) el.innerHTML = `<span class="mini-spinner"></span>${message}`;
}
function renderLayerToggles() {
  const el = document.getElementById("layer-toggles");
  if (!el) return;

  el.innerHTML = Object.entries(LAYER_META)
    .map(([key, [name, color, type]]) => {
      ...
      const helper = isChangeMap
        ? '<small class="layer-helper">4 kelas perubahan</small>'
        : "";
...
function updateComparisonOpacity(value) {
  const v = num(value, 50);
  document.getElementById("compare-value").textContent = `${v} : ${100 - v}`;
  ["vegetation2024", "vegetation2025"].forEach((k) => {
    const l = APP_STATE.layers[k];
    if (l)
      l.setStyle({
        fillOpacity: k === "vegetation2024" ? v / 100 : (100 - v) / 100,
      });
  });
}
function activeLayerArea(key) {
  if (
    ![
      "vegetation2024",
      "vegetation2025",
    ...
    return null;
  const selected = APP_STATE.selectedDistrict || "all";
  const stats = calculateDistrictStats(selected);
  return {
    vegetation2024: stats.luas_vegetasi_2024_ha,
    vegetation2025: stats.luas_vegetasi_2025_ha,
    gain: stats.luas_gain_ha,
    loss: stats.luas_loss_ha,
    priorityZones: stats.luas_loss_ha,
  }[key];
}
function updateActiveLayerStats() {
  const el = document.getElementById("active-layer-stats");
  if (!el) return;
  const active = [...APP_STATE.activeOverlays].filter(
    (k) => !["boundary", "districts"].includes(k),
  );
  if (!active.length) {
    el.innerHTML =
      '<span class="section-label">LAYER AKTIF</span><strong>Belum ada layer tematik</strong><small>Aktifkan layer untuk melihat statistik.</small>';
    return;
  }
  const key = active[active.length - 1],
    meta = LAYER_META[key],
    data = APP_STATE.data[key];
  const area = activeLayerArea(key);
  el.innerHTML = `<span class="section-label">LAYER AKTIF</span><strong>${meta[0]}</strong><small>${key === "changeMap" ? "GeoJSON empat kelas perubahan" : `${data?.features?.length || 0} fitur${area !== null ? ` • ${safeNumber(area)} ha` : ""}`}</small>`;
}
...
function downloadActiveLayers() {
  const keys = [...APP_STATE.activeOverlays].filter((k) => APP_STATE.data[k]);
  if (!keys.length)
    return alert("Aktifkan minimal satu layer vektor terlebih dahulu.");
  const features = [];
  keys.forEach((k) =>
    (APP_STATE.data[k].features || []).forEach((f) =>
      features.push({ ...f, properties: { ...f.properties, webgis_layer: k } }),
    ),
  );
  downloadBlob(
    "layer_aktif_webgis_ambon.geojson",
    JSON.stringify({ type: "FeatureCollection", features }, null, 2),
    "application/geo+json",
  );
}
function downloadStatistics() {
  const name = APP_STATE.selectedDistrict || "all",
    s = calculateDistrictStats(name);
  const rows = [
    ["wilayah", name === "all" ? "Kota Ambon" : name],
    ...Object.entries(s),
  ];
  downloadBlob(
    `statistik_${name.replaceAll(" ", "_")}.csv`,
    rows
      .map((r) =>
