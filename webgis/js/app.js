function fitMapToWholeAmbon() {
  ...
  const applyFit = () => {
    ...
    if (boundaryLayer?.getBounds?.().isValid()) {
      webgisMap.fitBounds(boundaryLayer.getBounds(), {
...
async function activateTab(id) {
  ...
  if (id === "evaluation-tab" && !APP_STATE.loadedTabs.has(id)) {
    renderEvaluation();
    APP_STATE.loadedTabs.add(id);
  }
  if (id === "insight-tab" && !APP_STATE.loadedTabs.has(id)) {
    const el = document.getElementById("district-ranking");
    if (el)
      el.innerHTML =
        '<div class="inline-loading"><span class="mini-spinner"></span> Menyiapkan analisis kecamatan…</div>';
...
function initializeTabs() {
  ...
  document
    ...
    .forEach((b) =>
      b.addEventListener("click", () => activateTab(b.dataset.openTab)),
    );
  document
...
window.runFocusButton = async function (key, button) {
  ...
  try {
    document
      .getElementById("focus-gain-button")
      ?.setAttribute("disabled", "disabled");
    document
      .getElementById("focus-loss-button")
      ?.setAttribute("disabled", "disabled");
    ...
    showLayerError(key === "gain" ? "Gain Vegetasi" : "Loss Vegetasi", error);
  } finally {
...
function initializeControls() {
  ...
  document
    .getElementById("download-active")
    ?.addEventListener("click", downloadActiveLayers);

  document
    .getElementById("download-stats")
    ?.addEventListener("click", downloadStatistics);
}
